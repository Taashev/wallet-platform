...
## Итерация 22 — 2026-04-22
**Фича:** TASK-022 — UI-hardening форм и экранов
**Статус:** Завершено
**Что сделано:**
- Усилен общий form layer: `DashboardField` теперь пробрасывает `aria-errormessage`, а `DashboardNotice` сообщает изменения через `aria-live`, чтобы ошибки и success/loading состояния читались предсказуемее
- На `Sign In`, `Sign Up`, `Edit Profile` и `Change Password` добавлены inline hints, `aria-busy`, autofocus на первом поле и очистка локальных ошибок при повторном вводе, чтобы сценарии с клавиатуры были короче и понятнее
- На `Users` добавлены более явные disabled/loading состояния для поиска и пагинации: toolbar блокируется на время запроса, результаты можно обновлять без визуального скачка, а refresh поверх готового списка сопровождается отдельным status-banner
- Подчищен responsive/focus слой: увеличены интерактивные зоны у form controls, улучшен focus у password toggle, а settings tabs на mobile превращены в более читаемые pill-navigation элементы вместо тонкой нижней линии
- Playwright e2e обновлены под новые CTA и дополнены keyboard-first сценарием для `Sign In`, а mobile smoke теперь явно проверяет доступность settings tabs на узкой ширине
**Следующие шаги:**
- Backlog frontend MVP закрыт; дальше логично переходить либо к post-MVP polish, либо к новым продуктовым задачам вне текущего `tasks.json`
**Заметки для следующей итерации:**
- `npm run lint`, `npm run build` и `npm run test:e2e` прошли успешно
- `npm run test:e2e` в sandbox сначала падал на `listen EPERM` для `127.0.0.1:4173`; для финальной проверки потребовался запуск с эскалацией, потому что проблема была в ограничении среды, а не в приложении

## Итерация 23 — 2026-04-22
**Фича:** post-MVP cleanup — удаление legacy preview и dead code
**Статус:** Завершено
**Что сделано:**
- Удалены неиспользуемые `preview/home/form` модули, которые больше не участвовали в runtime после перехода на реальные auth/profile/users экраны
- `AppRouter` упрощён: публичные маршруты больше не держат дублирующие child elements для `SignInPage` и `SignUpPage`, потому что текущий `PublicLayout` сам управляет обоими pane-состояниями
- Из `index.css` удалены стили для выпиленных экранов и старых shell-вариантов, чтобы не тащить legacy селекторы в production bundle
**Следующие шаги:**
- Если пойдёт следующий cleanup-проход, имеет смысл уже смотреть не на dead code, а на локальные state-машины страниц и повторяющиеся async-flow паттерны
**Заметки для следующей итерации:**
- Cleanup не меняет продуктовый scope и не затрагивает backend-контракты; это техническая уборка поверх уже закрытого MVP
